...
fn explore(
    ...
    counter: usize
        ...
        if *index >= 0 && *index <= 361 && board[*index as usize] == type_of_index {
            explore(board, direction, &(*index + direction), type_of_index, counter + 1)
        } else { counter }
}
...
macro_rules! check_winner {
    ($board:expr, $direction:expr, $index_lpiece:expr) => {
        explore($board, $direction, $index_lpiece, $board[*$index_lpiece as usize], 0) + 
            explore($board, -$direction, $index_lpiece, $board[*$index_lpiece as usize], 0) - 1 >= 5
    };
}
